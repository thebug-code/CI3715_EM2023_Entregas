...
def populate_db():
    ...
    roles = [
        user, admin, manager, mechanics_supervisor, painting_supervisor,
        mechanics_specialist, electronics_specialist, electricity_specialist
    ]
    ...
    admin_user = User(
        ...
        profiles.hash_password('Admin123.'), admin
    )
    ...
    stmt = db.select(User)
    for user, in db.session.execute(stmt):
        new_event = Event(user, 'Perfiles de Usuarios', f"Agregar usuario '{user.username}'")
